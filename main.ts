...
app.post("/login",async (req: Request, res: Response) => {
  ...
  const sql = `select * from users where name = '${username}' and password = '${password}';`
    ...
    res.json({message: "Login successful", nickname: row.nickname , userId :req.session.userId})
    return
...
app.post("/register",  async function (req: Request, res: Response) {
  ...
  const userResult = await pgClient.query(`Select * from users where name = '${username}'`)
